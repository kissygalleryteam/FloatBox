 ...
 * @module FloatBox
 **/
KISSY.add('kg/FloatBox/2.0.0/index',function (S, Node, DOM, Base) {
    ...
    S.extend(FloatBox, Base, /** @lends FloatBox.prototype*/{
        ...
        /**
         * 检查元素是否已经包含了placeholder，没有的话就构建
         * @param item
         */
        initPlaceholder: function( item ){

            item = Node.one( item );
            if( !item.data( 'fixedPlaceholder') ){
        ...
        scrollEventHandler: function() {
            ...
            return function(){
                ...
                // 先找出可能需要浮动的元素
                self.container.all( self.config.floats ).each(function( item ){

                    // 检查是否已经安装了placeholder
                    self.initPlaceholder( item );
